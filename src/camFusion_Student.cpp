...
void show3DObjects(std::vector<BoundingBox> &boundingBoxes, cv::Size worldSize, cv::Size imageSize, bool bWait)
{
    ...
    cv::resizeWindow(windowName, 10, 10);
                      ...
                      std::vector<cv::DMatch> kptMatches, double frameRate, double &TTC, cv::Mat *visImg)
{
   vector<double> distRatios; // stores the distance ratios for all keypoints between curr. and prev. frame
    ...
    // std:: cout<< "TTC in terms of Mean"<<std::endl;
    // }
    ...
    std:: cout<< "TTC in terms of Median"<<std::endl;
    }
}
...
void matchBoundingBoxes(std::vector<cv::DMatch> &matches, std::map<int, int> &bbBestMatches, DataFrame &prevFrame, DataFrame &currFrame)
{
    ...
    vector<vector<int>> boundingBoxMatchCount(prevFrame.boundingBoxes.size(),vector<int> (currFrame.boundingBoxes.size(),0));
    // boundingBoxMatchCount contains the count of matches between the different combination of bounding boxes in the previous and current frame
    // Idea is to find the maximum matches for the each previous frame bounding box with the current fram and store the id and points.

    // Iterating over  all keypoint matches between the prvious and current frame
    for(auto it1 = matches.begin(); it1< matches.end(); ++it1)
    {
        // extracting the index of the current match
        ...
        // Storing the point present in the region of interest 
        ...
        for (auto it2 = currFrame.boundingBoxes.begin(); it2 != currFrame.boundingBoxes.end(); ++it2)
        {
            ...
            {
                currBoundingBoxIds.push_back(it2->boxID);
            }
        }
        ...
        for (auto prevId : prevBoundingBoxIds)
        {
            ...
            {
                boundingBoxMatchCount[prevId][currId]++;
            }
        }
    }
    ...
    for (int prevId = 0; prevId < prevFrame.boundingBoxes.size(); ++prevId)
    {
        ...
        {
            if (boundingBoxMatchCount[prevId][currId] > maxCount)
            {
                maxCount = boundingBoxMatchCount[prevId][currId];
                maxId = currId;
            }
        }
